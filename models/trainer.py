...
class CDTrainer():
    ...
    def _forward_pass_dp(self, batch):
        ...
        img_in1 = batch['A'].to(self.device)
        img_in2 = batch['B'].to(self.device)
        self.G_pred, self.ms_pred = self.net_G(img_in1, img_in2)
    ...
    def _backward_G_dp(self):
        gt = self.batch['L'].to(self.device).long()
        self.G_loss = self._pxl_loss(self.G_pred, gt)

        temp_loss = 0.0
        for pred in self.ms_pred:
            if pred.size(2) != gt.size(2):
                temp_gt = F.interpolate(self.batch['L'].to(self.device).float(), size=pred.size(2), mode="nearest")
                temp_loss = temp_loss + self._pxl_loss(pred, temp_gt.long())
            else:
