...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
        ...
        self.csst_c4 = CSSTransformerLevel(block_size=4, num_heads=self.embedding_dim//64,
                                         depth=2, embed_dim=self.embedding_dim, norm_layer=nn.LayerNorm,
                                         act_layer=nn.GELU)
        self.csst_c3 = CSSTransformerLevel(block_size=4, num_heads=self.embedding_dim // 64,
                                         depth=2, embed_dim=self.embedding_dim, norm_layer=nn.LayerNorm,
                                         act_layer=nn.GELU)
        self.csst_c2 = CSSTransformerLevel(block_size=4, num_heads=self.embedding_dim // 64,
                                         depth=2, embed_dim=self.embedding_dim, norm_layer=nn.LayerNorm,
                                         act_layer=nn.GELU)
        self.csst_c1 = CSSTransformerLevel(block_size=4, num_heads=self.embedding_dim // 64,
                                         depth=2, embed_dim=self.embedding_dim, norm_layer=nn.LayerNorm,
                                         act_layer=nn.GELU)
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4_1 = self.linear_c4(c4_1).permute(0, 2, 1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3])
        _c4_2 = self.linear_c4(c4_2).permute(0, 2, 1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4 = self.diff_c4(torch.cat((_c4_1, _c4_2), dim=1)) + torch.abs(_c4_1-_c4_2)
        _c4, p_c4 = self.cat_c4(_c4)
        ...
        _c3 = _c3 + torch.abs(_c3_1-_c3_2)
        _c3, p_c3 = self.cat_c3(_c3)
        ...
        _c2 = _c2 + torch.abs(_c2_1 - _c2_2)
        _c2, p_c2 = self.cat_c2(_c2)
        ...
        _c1 = _c1 + torch.abs(_c1_1 - _c1_2)
        _c1, p_c1 = self.cat_c1(_c1)
...
# ChangeFormerV6:
class CSSTChangeFormerV6(nn.Module):

    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        super(CSSTChangeFormerV6, self).__init__()
        # Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
...
class CSSTransformerBlock(nn.Module):
    def __init__(self, dim, proj_dim, num_heads, mlp_ratio=4., qkv_bias=False,
        ...
        self.attn = Attentioncsst(
            proj_dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        mlp_hidden_dim = int(proj_dim * mlp_ratio)
        self.mlp = Mlpcsst(in_features=proj_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, q, kv):
        # print('-----2', q.shape, kv.shape)
...
for k in a['model_G_state_dict'].keys():
    if 'cat' in k:
        new_k=k.replace('ca', 'css')
        a['model_G_state_dict'][new_k]=a['model_G_state_dict'].pop(k)
        print(k)

new_state_dict = OrderedDict()
for k,v in a['model_G_state_dict'].items():
    name = k.replace('ca', 'css') if 'cat' in k else k
    new_state_dict[name] = v
    a['model_G_state_dict']=new_state_dict